...
#define WPS_DEFAULT_CONNECT_STATUS_COUNT    5
...
void swc_connection_set_event_callback(swc_connection_t *conn, void (*cb)(void *conn))
{
...
wps_error_t swc_connection_send(swc_connection_t *conn, uint8_t *payload_buffer, uint8_t size, swc_error_t *err)
{
    wps_error_t wps_err;
...
swc_error_t swc_get_event_error(swc_connection_t *conn)
{
    ...
    case WPS_PHY_CRITICAL_ERROR:
        return SWC_ERR_CRITICAL;
    default:
        return SWC_ERR_NONE;
    }
}
