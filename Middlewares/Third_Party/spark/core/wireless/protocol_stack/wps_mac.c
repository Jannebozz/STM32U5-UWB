...
static void state_post_rx(void *signal_data)
{
    ...
    uint8_t gain_index;
    connect_status_t status = wps_mac->current_timeslot->connection_main->connect_status.status;

    link_update_connect_status(&wps_mac->current_timeslot->connection_main->connect_status, wps_mac->current_xlayer->frame.frame_outcome);

    if (link_connect_get_status(&wps_mac->current_timeslot->connection_main->connect_status, &status)) {
        ...
        wps_callback_enqueue(wps_mac->callback_queue, wps_mac->current_xlayer);
        wps_mac->current_timeslot->connection_main->wps_event = (status == CONNECT_STATUS_CONNECTED)
                                                                    ? WPS_EVENT_CONNECT
...
static void state_post_tx(void *signal_data)
{
    ...
    if (wps_mac->current_xlayer != &wps_mac->empty_frame_tx) {
        if (outcome_is_tx_sent_ack(wps_mac)) {
            ...
            send_done(wps_mac->current_timeslot->connection_main);
        } else {
            if (is_current_timeslot_tx(wps_mac)) {
                current_connection = wps_mac->current_timeslot->connection_main;
                current_gain_loop =
                    wps_mac->current_timeslot->connection_main->gain_loop[wps_mac->current_channel_index];
                current_lqi = &wps_mac->current_timeslot->connection_main->lqi;
                current_channel_lqi =
                    &wps_mac->current_timeslot->connection_main->channel_lqi[wps_mac->current_channel_index];
                if (!wps_mac->current_timeslot->connection_main->ack_enable) {
                    ...
                    if (!is_saw_arq_enable(wps_mac->current_timeslot->connection_main)) {
                        send_done(wps_mac->current_timeslot->connection_main);
                    }
                }
            } else {
                ...
                if (outcome_is_tx_not_sent(wps_mac)) {
                    ...
                    wps_mac->current_xlayer->config.parg_callback =
                        wps_mac->current_timeslot->connection_auto_reply->tx_success_parg_callback_t;
                    send_done(wps_mac->current_timeslot->connection_auto_reply);
                }
            }
        }
        gain_index = link_gain_loop_get_gain_index(current_gain_loop);
...
static void state_setup_primary_link(void *signal_data)
{
    ...
    wps_mac->main_xlayer->config.channel = &wps_mac->current_timeslot->connection_main->channel[fallback_index][next_channel][MULTI_RADIO_BASE_IDX];
...
static void state_setup_prime_link(void *signal_data)
{
    ...
    if( wps_mac->auto_xlayer != NULL){
        ...
        wps_mac->auto_xlayer->config.channel = &wps_mac->current_timeslot->connection_main->channel[fallback_index][next_channel][MULTI_RADIO_BASE_IDX];
...
static xlayer_t *get_xlayer_for_tx(wps_mac_t *wps_mac, wps_connection_t *connection)
{
    ...
    connect_status_t connect_status = wps_mac->current_timeslot->connection_main->connect_status.status;

    link_connect_get_status(&wps_mac->current_timeslot->connection_main->connect_status, &connect_status);
    ...
    if (!connection->auto_sync_enable && free_xlayer == NULL) {
        /* Dont send anything */
     ...
     } else if ((connect_status == CONNECT_STATUS_DISCONNECTED || free_xlayer == NULL) && connection->ack_enable == true) {
        /* Send sync frame */
