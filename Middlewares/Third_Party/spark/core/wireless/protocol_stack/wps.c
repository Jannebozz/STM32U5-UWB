...
//Due to DMA and DCache, we need to extract these and force them to 32-byte aligning (and multiples of 32bytes).
#define UWB_RADIO_SPI_BUFFER_SIZE 224
__attribute__((aligned(32))) uint8_t uwb_spi_rx_buffer[UWB_RADIO_SPI_BUFFER_SIZE];
__attribute__((aligned(32))) uint8_t uwb_spi_tx_buffer[UWB_RADIO_SPI_BUFFER_SIZE];
...
void wps_connection_config_status(wps_connection_t *connection, connect_status_cfg_t *status_cfg, wps_error_t *err)
{
    *err = WPS_NO_ERROR;
    ...
    link_connect_status_init(status_cfg, &connection->connect_status);
}
