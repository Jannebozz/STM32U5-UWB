...
static void error(wps_t *wps)
{
    ...
    wps->mac.current_timeslot->connection_main->wps_event = WPS_EVENT_ERROR;
...
static void yield(wps_t *wps)
{
    ...
    if (request != NULL) {
        process_pending_request(wps, request);
...
static void set_signal_mac_to_wps(wps_mac_t *mac, wps_t *wps)
{
    ...
    case MAC_SIGNAL_WPS_FRAME_RX_OVERRUN:
        mac->current_timeslot->connection_main->wps_event = WPS_EVENT_ERROR;
