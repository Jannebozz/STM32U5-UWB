...
static wps_phy_state_t prepare_radio_cut_through_states[] = {set_header, set_payload_cut_through, prepare_radio, signal_yield,
...
static wps_phy_state_t syncing_states[]                   = {close_spi, enable_radio_irq, read_events_syncing,
                                                             close_spi, process_event_rx, end};
...
static void prepare_radio_tx(wps_phy_t *phy, int_flag_cfg_t *radio_events)
{
    ...
    if (phy->xlayer_auto != NULL) {
        ...
        *radio_events        = set_events_for_tx_with_ack();
        rx_packet_size       = phy->xlayer_auto->frame.payload_memory_size;
        /* Ack mode */
...
/** @brief Sub function to prepare a recieve frame.
 *
 *  @param[in] signal_data   Data required to process the state. The type shall be wps_phy_t.
 *  @param[in] radio_events  Radio's interrupt events enumeration.
 *  @param[in] radio_actions   Radio actions instance.
...
static void prepare_radio_rx(wps_phy_t *phy, int_flag_cfg_t *radio_events)
{
    ...
    if (phy->xlayer_auto != NULL) {
        ...
        *radio_events                = set_events_for_rx_with_ack();
        ...
        phy->cfg.source_address      = &phy->xlayer_auto->config.source_address;
        phy->cfg.channel             = phy->xlayer_auto->config.channel;
...
static void set_channel_config(wps_phy_t *phy)
{
    ...
    uwb_select_channel(phy->radio,
                       ...
                       phy->xlayer_main->config.channel->pulse_size);
    uwb_transfer_non_blocking(phy->radio);
}
...
static void handle_good_auto_reply(wps_phy_t *phy)
{
    ...
    if (!phy->cfg.fixed_payload_size_enable) {
        ...
        *phy->rx_frame_size = 0;
        uwb_transfer_non_blocking(phy->radio);
        enqueue_states(phy, end_tx_cut_through_states);
        enqueue_states(phy, prepare_phy_states);
    }
}
    ...
    if (*phy->rx_frame_size == 0) {
        ...
        *phy->rx.signal               = PHY_SIGNAL_FRAME_MISSED;
        ...
        sr_access_spi_transfer_non_blocking(phy->radio->radio_hal,
                                            phy->radio->access_sequence.tx_buffer,
...
static void get_payload_cut_through(wps_phy_t *phy)
{
    ...
    if (phy->partial_frame_index == 0) {
        partial_frame_len = phy->rx.payload_size / phy->partial_frame_count +
                            phy->rx.payload_size % phy->partial_frame_count +
...
static void prepare_syncing(wps_phy_t *phy)
{
    ...
    uwb_set_radio_actions(phy->radio, SET_RADIO_ACTIONS(RADIO_ACTIONS_CLEAR,
                                                        ...
                                                        FLUSH_TX_RESET_TX_BUFFER,
                                                        GO_TO_SLEEP,
                                                        RX_MODE_ENABLE_RECEPTION));

    uwb_set_rx_filters_raw(phy->radio, phy->xlayer_main->config.channel->channel.rx_filter);
    uwb_select_channel(phy->radio,
                       (uint8_t *)phy->cfg.channel->channel.tx_pattern,
                       phy->cfg.channel->pulse_size);
